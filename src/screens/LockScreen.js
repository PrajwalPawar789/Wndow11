...
const LockScreen = () => {
  ...
  const changePINHandler = ({ target }) => {
    ...
    if (PIN.length === CONFIGS.pinLimit) {
      if (PIN === CONFIGS.validPIN) {
  ...
  return (
    <>
        ...
        >
          <div className="mt-28 flex flex-col items-center justify-center">
            <img
              ...
              alt="User Profile"
            />
            <h1 className="mt-4 text-2xl font-semibold">Pooja Kothawale</h1>
            {loginIsSuccess ? (
      ...
      {helpForgotPIN && (
        ...
        >
          <NotificationFrom
            from="Pooja Kothawale"
            image={profiles.pooja}
            message={
              <>
                Hello There!
                <br /> The Secret PIN is{" "}
                <span className="font-semibold">{CONFIGS.validPIN}</span>
              </>
            }
          />
        </motion.div>
